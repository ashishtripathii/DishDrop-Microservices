...
import { restaurantService, utilsService } from "../config/config";
...
const CheckOut = () => {
  ...
  const payWithRazorpay = async () => {
    try {
      ...
      console.log(error);
      toast.error("Payment failed please refresh paga");
    } finally {
    ...
    <div className="mx-auto max-w-4xl px-4 py-6 space-y-6">
          ...
          addresses.map((add) => (
            <label
              ...
              className={`flex gap-3 rounded-lg border p-3 cursor-pointer transition
                ${selectedAddressId === add._id ? "border-[#e23774] bg-red" : "hover:bg-gray-50"} `}
            >
              <input
                type="radio"
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm space-y-3 ">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#e23345] py-3 text-sm font-semibold 
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#2D7FF9] py-3 text-sm font-semibold 
